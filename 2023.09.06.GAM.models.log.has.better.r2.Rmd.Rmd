...
matrix_TP<-TP_matrix %>% 
  select(LAKE_HISTORY_ID,depth,DISTURBANCE,DISTURBANCE100m,logTP,well_drained_soils,WLarea) %>% distinct() %>% 
...
only_distdepth <- gam(logTP~s(DISTURBANCE)+s(depth), data=TP_matrix %>% filter(!is.na(depth)))
...
distdepth<-TP_matrix %>% select(depth,DISTURBANCE,logTP) %>% filter(!is.na(depth),!is.na(DISTURBANCE))
...
print(ggplot()+
        geom_point(data=distdepth,aes(x=DISTURBANCE,y=prediction))+
        geom_smooth(data=distdepth %>% filter(depth<5),aes(x=DISTURBANCE,y=prediction))+
        geom_smooth(data=distdepth %>% filter(depth>=5&depth<10),aes(x=DISTURBANCE,y=prediction))+
        geom_smooth(data=distdepth %>% filter(depth>=10&depth<20),aes(x=DISTURBANCE,y=prediction))+
        geom_smooth(data=distdepth %>% filter(depth>=20),aes(x=DISTURBANCE,y=prediction))+
        geom_hline(yintercept = 20)+
