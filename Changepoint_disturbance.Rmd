...
landcover_2016<-landcover_2016 %>% 
  mutate(DEVELOPED=VALUE_21+VALUE_22+VALUE_23+VALUE_24,
         DISTURBANCE=VALUE_21+VALUE_22+VALUE_23+VALUE_24+VALUE_31+VALUE_81+VALUE_82) %>% 
  select(Lake_ID,DEVELOPED,DISTURBANCE) %>% distinct() %>% 
...
###### if one is better than the other.
   ...
   for(i in 1:(m-1)) {
      vi[i] <- sum((TP[DISTURBANCE <= mx[i]] - mean(TP[DISTURBANCE <=
               mx[i]]))^2) + sum((TP[DISTURBANCE > mx[i]] - mean(
               TP[DISTURBANCE > mx[i]]))^2)
              }
   chngp <- mean(mx[vi == min(vi)])

   F.stat <- (vi[m]-min(vi))*(length(DISTURBANCE)-2)/(vi[m]*2)
   p.value <- 1-pf(F.stat, 2, length(DISTURBANCE)-2)
   out <- c(chngp, p.value, mean(TP[DISTURBANCE <= chngp]), mean(TP[DISTURBANCE > chngp]))
...
#Warning: `bootstrap()` is deprecated and will be removed in an upcoming release of broom. See the rsample package instead.
#Error in by_group && !is.null(groups(df)) : invalid 'x' type in 'x && y'

hist(results$thetastar,breaks=100, col="darkgray",
    main="Histogram of Bootstrap Changepoints", xlab = "% DISTURBANCE Land Cover")
...
plot (TP,DISTURBANCE, ylab="% DISTURBANCE Land Cover",bty="l", col=circlecol, pch=circle,xlab="TP (ug/L)")
if(lowessline=="Y"){lines(lowess(DISTURBANCE~TP), lwd=lowesslwd, col=lowesscol,lty=lowesslty)}
