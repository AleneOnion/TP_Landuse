...
habs <- status %>%
  ...
  select(LAKE_HISTORY_ID,LOCATION_HISTORY_ID, SAMPLE_DATE, SAMPLE_TIME, INFORMATION_TYPE, SAMPLE_TYPE, DATA_PROVIDER, HS_HAB_STATUS, HS_HAB_STATUS_DATE, HS_HAB_STATUS_REMARK) %>%distinct(SAMPLE_DATE, .keep_all = TRUE) %>% 
...
write.csv(habs,file="junk.GC.for.hab.layer.csv",row.names=FALSE)